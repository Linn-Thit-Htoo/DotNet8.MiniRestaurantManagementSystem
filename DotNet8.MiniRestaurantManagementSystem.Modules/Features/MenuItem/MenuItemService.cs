...
public class MenuItemService : IMenuItemService
{
    ...
    )
    {
        ...
        try
        {
            var lst = await _context
                .TblMenuItems.OrderByDescending(x => x.MenuItemId)
    ...
    public async Task<Result<IEnumerable<MenuItemDto>>> GetMenuItemsByCategoryCodeAsync(
        string categoryCode,
        CancellationToken cancellationToken
    )
    {
        ...
        {
            var lst = await _context
                .TblMenuItems.Where(x => x.CategoryCode == categoryCode)
    ...
    public async Task<Result<MenuItemDto>> GetMenuItemAsync(
        int id,
        CancellationToken cancellationToken
    )
    {
        ...
        try
        {
            var menuItem = await _context.TblMenuItems.FindAsync(
    ...
    public async Task<Result<MenuItemDto>> CreateMenuItemAsync(
    ...
    )
    {
        ...
        try
        {
            bool categoryValid = await _context.TblCategories.AnyAsync(
                x => x.CategoryCode == menuItemDto.CategoryCode,
                cancellationToken: cancellationToken
            );
            if (!categoryValid)
            {
            ...
            await _context.TblMenuItems.AddAsync(
                menuItemDto.ToEntity(),
                cancellationToken: cancellationToken
            );
    ...
    public async Task<Result<MenuItemDto>> DeleteMenuItemAsync(
        int id,
        CancellationToken cancellationToken
    )
    {
