...
public class OrderService : IOrderService
{
    ...
    public async Task<Result<OrderListDto>> GetOrdersAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var orderLst = await _context
                .TblOrders.OrderByDescending(x => x.OrderId)
            ...
            {
                var detailLst = await _context
                    .TblOrderDetails.Where(x => x.InvoiceNo == order.InvoiceNo)
                    .ToListAsync(cancellationToken: cancellationToken);
    ...
    public async Task<Result<OrderDataDto>> ViewOrderAsync(
        string invoiceNo,
    ...
    )
    {
        ...
        try
        {
            var order = await _context.TblOrders.FirstOrDefaultAsync(
                x => x.InvoiceNo == invoiceNo,
                cancellationToken: cancellationToken
            );
            ...
            var detailLst = await _context
                .TblOrderDetails.Where(x => x.InvoiceNo.Equals(order.InvoiceNo))
    ...
    )
    {
        ...
        try
        {
            ...
            {
                await _context.TblOrderDetails.AddAsync(
                    item.ToEntity(order.InvoiceNo),
                    cancellationToken
                );
            }
